...
<body>
  <main class="shell">
    ...
    <div class="filter-area">
      ...
      <section class="filters-panel" id="filters-panel" aria-label="Map filters">
        <div class="filter-section"><h2>Category</h2><div class="filter-options">${categories.map((item) => checkbox("category", item, categoryCounts[item])).join("")}</div></div>
        <div class="filter-section"><h2>Countries</h2><div class="filter-options">${countries.map((item) => checkbox("country", item, countryCounts[item])).join("")}</div></div>
      </section>
    </div>
  ...
  <script>
    ...
    function addBufferedTileLayer(url,attribution=""){const tileLayer=L.tileLayer(url,{maxNativeZoom:16,maxZoom:20,keepBuffer:4,updateWhenIdle:false,attribution});const getVisiblePixelBounds=tileLayer._getTiledPixelBounds.bind(tileLayer);tileLayer._getTiledPixelBounds=center=>{const visibleBounds=getVisiblePixelBounds(center),edgeBuffer=tileLayer.getTileSize();return L.bounds(visibleBounds.min.subtract(edgeBuffer),visibleBounds.max.add(edgeBuffer))};tileLayer.addTo(map)}
    addBufferedTileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Dark_Gray_Base/MapServer/tile/{z}/{y}/{x}",'&copy; Esri, HERE, Garmin, <a href="https://www.openstreetmap.org/copyright">OpenStreetMap contributors</a>');
    addBufferedTileLayer("https://server.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Dark_Gray_Reference/MapServer/tile/{z}/{y}/{x}");
    ...
    document.querySelectorAll("[data-filter]").forEach(input=>input.addEventListener("change",()=>{const target=input.dataset.filter==="category"?selectedCategories:selectedCountries;input.checked?target.add(input.value):target.delete(input.value);renderMarkers()}));
