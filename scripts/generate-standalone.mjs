...
<body>
  ...
  <script type="module">
    ...
    function makeTitleLabelBackground(){const width=40,height=36,canvas=document.createElement("canvas");canvas.width=width;canvas.height=height;const context=canvas.getContext("2d");if(!context)return null;context.beginPath();context.roundRect(1.5,1.5,width-3,27,7);context.fillStyle="rgba(10, 24, 32, 0.82)";context.fill();context.lineWidth=1.5;context.strokeStyle="rgba(192, 92, 255, 0.32)";context.stroke();context.beginPath();context.moveTo(16.5,28);context.lineTo(20,34);context.lineTo(23.5,28);context.closePath();context.fillStyle="rgba(10, 24, 32, 0.82)";context.fill();context.strokeStyle="rgba(192, 92, 255, 0.32)";context.stroke();return context.getImageData(0,0,width,height)}

    let mapReady=false;
    map.on("load",()=>{
      addTopStar();
      const titleBackground=makeTitleLabelBackground();if(titleBackground)map.addImage("title-label-background",titleBackground,{pixelRatio:2,stretchX:[[7,16],[24,33]],stretchY:[[7,20]],content:[7,5,33,25]});
      ...
      map.addLayer({id:"clip-clusters",type:"circle",source:"clips",filter:["has","point_count"],paint:{"circle-radius":["step",["get","point_count"],16,10,19,100,22],"circle-color":"#34224f","circle-stroke-color":"#dc8cff","circle-stroke-width":2}});
      map.addLayer({id:"cluster-count",type:"symbol",source:"clips",filter:["has","point_count"],layout:{"text-field":["to-string",["get","point_count"]],"text-font":["Arial"],"text-size":["step",["get","point_count"],12,100,11],"text-allow-overlap":true,"text-ignore-placement":true,"text-rotation-alignment":"viewport","text-pitch-alignment":"viewport"},paint:{"text-color":"#fff","text-halo-color":"rgba(21, 10, 34, .55)","text-halo-width":.7}});
      ...
      map.addLayer({id:"clip-labels",type:"symbol",source:"clips",filter:["!",["has","point_count"]],layout:{visibility:"none","icon-image":"title-label-background","icon-text-fit":"both","icon-text-fit-padding":[5,8,5,8],"icon-allow-overlap":true,"icon-ignore-placement":true,"text-field":["get","name"],"text-font":["Arial"],"text-size":11,"text-anchor":"bottom","text-offset":[0,-1.25],"text-max-width":18,"text-allow-overlap":true,"text-ignore-placement":true,"text-rotation-alignment":"viewport","text-pitch-alignment":"viewport","icon-rotation-alignment":"viewport","icon-pitch-alignment":"viewport"},paint:{"text-color":"#efffff","text-halo-color":"rgba(4, 11, 16, 0.45)","text-halo-width":.7,"text-halo-blur":.2}});
      map.addLayer({id:"active-clip-point",type:"circle",source:"active-clip",filter:["==",["get","top"],false],paint:{"circle-radius":["case",["get","linked"],9.5,7.5],"circle-color":["case",["get","linked"],"#c86cff","#91a5ac"],"circle-stroke-color":"#f0c4ff","circle-stroke-width":2,"circle-blur":.06}});
      map.addLayer({id:"active-top-point",type:"symbol",source:"active-clip",filter:["==",["get","top"],true],layout:{"icon-image":"top-star","icon-size":1.8,"icon-allow-overlap":true}});
      map.addLayer({id:"clip-hit-area",type:"circle",source:"clips",filter:["!",["has","point_count"]],paint:{"circle-radius":["case",["get","top"],18,["get","linked"],11.5,10.5],"circle-color":"rgba(0, 0, 0, 0.01)","circle-stroke-width":0}});
      map.addLayer({id:"active-cluster",type:"circle",source:"active-cluster",paint:{"circle-radius":["step",["get","point_count"],17.5,10,20.5,100,23.5],"circle-color":"#4a2d6d","circle-stroke-color":"#f0b0ff","circle-stroke-width":2.3}});
      map.addLayer({id:"active-cluster-count",type:"symbol",source:"active-cluster",layout:{"text-field":["to-string",["get","point_count"]],"text-font":["Arial"],"text-size":["step",["get","point_count"],12,100,11],"text-allow-overlap":true,"text-ignore-placement":true,"text-rotation-alignment":"viewport","text-pitch-alignment":"viewport"},paint:{"text-color":"#fff","text-halo-color":"rgba(21, 10, 34, .65)","text-halo-width":.8}});
      const popup=new maplibregl.Popup({closeButton:false,closeOnClick:false,offset:12,className:"clip-map-tooltip"});
      function bindPointLayer(layerId){map.on("mouseenter",layerId,event=>{map.getCanvas().style.cursor="pointer";const feature=event.features&&event.features[0];if(!feature||feature.geometry.type!=="Point")return;mapHoveredPlace=places.find(item=>item.id===Number(feature.properties.id))||null;updateHighlightedPoint();popup.setLngLat(feature.geometry.coordinates).setText(feature.properties.name||"Névtelen klip").addTo(map)});map.on("mouseleave",layerId,()=>{map.getCanvas().style.cursor="";mapHoveredPlace=null;updateHighlightedPoint();popup.remove()});map.on("click",layerId,event=>{const id=Number(event.features&&event.features[0]&&event.features[0].properties.id),place=places.find(item=>item.id===id);if(place&&place.clipUrl)openClip(place)})}
      ...
      map.on("click","clip-clusters",event=>{const feature=event.features&&event.features[0];if(!feature||feature.geometry.type!=="Point")return;const clusterId=Number(feature.properties.cluster_id),pointCount=Number(feature.properties.point_count||2);map.getSource("clips").getClusterLeaves(clusterId,pointCount,0).then(leaves=>{const coordinates=leaves.filter(leaf=>leaf.geometry.type==="Point").map(leaf=>[Number(leaf.geometry.coordinates[0]),Number(leaf.geometry.coordinates[1])]);if(!coordinates.length)return;const west=Math.min(...coordinates.map(([longitude])=>longitude)),east=Math.max(...coordinates.map(([longitude])=>longitude)),south=Math.min(...coordinates.map(([,latitude])=>latitude)),north=Math.max(...coordinates.map(([,latitude])=>latitude)),container=map.getContainer(),base=Math.max(48,Math.round(Math.min(container.clientWidth,container.clientHeight)*.12)),options=getListAwareMapOptions(base);west===east&&south===north?map.easeTo({center:[west,south],zoom:17,offset:options.offset,duration:720}):map.fitBounds([[west,south],[east,north]],{padding:options.padding,maxZoom:17,duration:720})}).catch(()=>{})});
      map.on("click",event=>{const interactiveFeatures=map.queryRenderedFeatures(event.point,{layers:["clip-clusters","cluster-count","clip-hit-area","clip-points","top-points","clip-labels","active-clip-point","active-top-point","active-cluster","active-cluster-count"]});if(!interactiveFeatures.length)setListOpen(false)});
    ...
    function getListAwareMapOptions(base=54){const rect=getMapContentRect(),container=map.getContainer(),width=container.clientWidth,height=container.clientHeight;return{offset:[(rect.left+rect.right-width)/2,(rect.top+rect.bottom-height)/2],padding:{top:base+rect.top,right:base+width-rect.right,bottom:base+height-rect.bottom,left:base+rect.left}}}
    ...
    const titleToggle=document.getElementById("title-toggle"),filterButton=document.getElementById("filter-button"),filtersPanel=document.getElementById("filters-panel"),filterDismiss=document.getElementById("filter-dismiss");
    titleToggle.addEventListener("click",()=>{showTitles=!showTitles;titleToggle.classList.toggle("active",showTitles);titleToggle.setAttribute("aria-checked",String(showTitles));if(mapReady)map.setLayoutProperty("clip-labels","visibility",showTitles?"visible":"none")});
