...
<body>
  ...
  <script>
    ...
    const mapLoading=document.getElementById("map-loading");let hideLoadingTimer=0,coverageTimer=0,coverageFrame=0,viewTransitioning=false;
    function showMapLoading(){clearTimeout(hideLoadingTimer);mapLoading.classList.add("visible")}
    ...
    function verifyMapCoverage(){clearTimeout(coverageTimer);if(viewTransitioning||!visibleBaseTilesReady()){showMapLoading();coverageTimer=setTimeout(verifyMapCoverage,80);return}clearTimeout(hideLoadingTimer);hideLoadingTimer=setTimeout(()=>{if(viewTransitioning||!visibleBaseTilesReady())verifyMapCoverage();else mapLoading.classList.remove("visible")},80)}
    ...
    function addBufferedTileLayer(url,attribution="",options={},resilient=false){
      const tileLayer=L.tileLayer(url,{maxZoom:20,keepBuffer:4,updateWhenIdle:false,updateInterval:120,attribution,...options}),getVisiblePixelBounds=tileLayer._getTiledPixelBounds.bind(tileLayer);
      tileLayer._getTiledPixelBounds=center=>{const visibleBounds=getVisiblePixelBounds(center),edgeBuffer=tileLayer.getTileSize();return L.bounds(visibleBounds.min.subtract(edgeBuffer),visibleBounds.max.add(edgeBuffer))};
      if(resilient)tileLayer.createTile=(coords,done)=>{const size=tileLayer.getTileSize(),tile=document.createElement("div"),subdomains=["a","b","c","d"],retina=L.Browser.retina?"@2x":"";let finished=false;tile.className="resilient-map-tile";tile.style.width=size.x+"px";tile.style.height=size.y+"px";tile.dataset.tileX=String(coords.x);tile.dataset.tileY=String(coords.y);tile.dataset.tileZ=String(coords.z);
        const tileUrl=(x,y,z,attempt)=>{const subdomain=subdomains[Math.abs(x+y+attempt)%subdomains.length],resolved=url.replace("{s}",subdomain).replace("{z}",String(z)).replace("{x}",String(x)).replace("{y}",String(y)).replace("{r}",retina);return attempt?resolved+"?_tileRetry="+Date.now()+"-"+attempt:resolved};
        const complete=(image,fallbackLevel)=>{if(finished)return;finished=true;const factor=2**fallbackLevel,offsetX=((coords.x%factor)+factor)%factor,offsetY=((coords.y%factor)+factor)%factor;image.style.width=size.x*factor+"px";image.style.height=size.y*factor+"px";image.style.left=-offsetX*size.x+"px";image.style.top=-offsetY*size.y+"px";done(null,tile)};
        const load=(fallbackLevel,attempt=0)=>{const factor=2**fallbackLevel,x=Math.floor(coords.x/factor),y=Math.floor(coords.y/factor),z=coords.z-fallbackLevel,image=document.createElement("img");image.alt="";image.decoding="async";image.onload=()=>complete(image,fallbackLevel);image.onerror=()=>{if(attempt<2)load(fallbackLevel,attempt+1);else if(z>0)load(fallbackLevel+1);else if(!finished)done(new Error("Map tile unavailable"),tile)};tile.replaceChildren(image);image.src=tileUrl(x,y,z,attempt)};load(0);return tile};
      if(resilient)tileLayer.on("tileload tileerror",requestCoverageCheck);tileLayer.addTo(map)
    }
    addBufferedTileLayer("https://{s}.basemaps.cartocdn.com/dark_nolabels/{z}/{x}/{y}{r}.png",'&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> &copy; <a href="https://carto.com/attributions">CARTO</a>',{subdomains:"abcd"},true);
    ...
    map.on("move",requestCoverageCheck);map.on("moveend",requestCoverageCheck);map.on("zoomstart",()=>{viewTransitioning=true;showMapLoading()});map.on("zoomend",()=>{viewTransitioning=false;requestCoverageCheck()});
