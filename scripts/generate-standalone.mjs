...
const checkbox = (type, item) => `<label><input type="checkbox" data-filter="${type}" value="${escapeAttribute(item)}" checked><span>${escapeAttribute(item)}</span></label>`;
...
<head>
  ...
  <style>
    .shell{isolation:isolate}#map{position:absolute;inset:0;z-index:0;isolation:isolate}.header,.filter-area,.hint{transform:translate3d(0,0,0);backface-visibility:hidden}
  </style>
</head>
<body>
  <main class="shell">
    ...
    <div class="filter-area">
      ...
      <section class="filters-panel" id="filters-panel" aria-label="Map filters">
        <div class="filter-section"><h2>Category</h2><div class="filter-options">${categories.map((item) => checkbox("category", item)).join("")}</div></div>
        <div class="filter-section"><h2>Countries</h2><div class="filter-options">${countries.map((item) => checkbox("country", item)).join("")}</div></div>
      </section>
    </div>
