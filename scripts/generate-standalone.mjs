...
<body>
  ...
  <script type="module">
    ...
    const prefetchedTileUrls=new Set(),tilePrefetchQueue=[];let activeTilePrefetches=0,prefetchTimer=0,flashTimer=0,flashRestoreTimer=0;
    ...
    function scheduleNodeFlash(){clearTimeout(flashTimer);clearTimeout(flashRestoreTimer);flashTimer=setTimeout(()=>{if(!map.getLayer("clip-points")||!map.getLayer("top-points"))return;map.setPaintProperty("clip-points","circle-color",["case",["get","linked"],"#d895ff","#7c9299"]);map.setLayoutProperty("top-points","icon-image","top-star-flash");flashRestoreTimer=setTimeout(()=>{map.setPaintProperty("clip-points","circle-color",["case",["get","linked"],"#bd5cff","#7c9299"]);map.setLayoutProperty("top-points","icon-image","top-star")},120)},80)}
    function wakeMap(){requestAnimationFrame(()=>requestAnimationFrame(()=>{map.resize();map.triggerRepaint();scheduleTilePrefetch()}))}
    new ResizeObserver(wakeMap).observe(document.getElementById("map"));window.addEventListener("load",wakeMap);window.addEventListener("pageshow",wakeMap);document.addEventListener("visibilitychange",()=>{if(document.visibilityState==="visible")wakeMap()});map.on("moveend",scheduleTilePrefetch);map.on("moveend",syncListViewport);map.on("zoomend",scheduleNodeFlash);map.on("resize",syncListViewport);map.on("idle",scheduleTilePrefetch);
    ...
    function addTopStar(name="top-star",flash=false){const size=80,canvas=document.createElement("canvas");canvas.width=size;canvas.height=size;const context=canvas.getContext("2d");if(!context)return;context.scale(2,2);const outer=17,inner=7.4,center=20;context.beginPath();for(let index=0;index<10;index+=1){const radius=index%2===0?outer:inner,angle=-Math.PI/2+index*Math.PI/5,x=center+Math.cos(angle)*radius,y=center+Math.sin(angle)*radius;index===0?context.moveTo(x,y):context.lineTo(x,y)}context.closePath();const gradient=context.createLinearGradient(8,6,31,34);gradient.addColorStop(0,flash?"#ffe0ff":"#f1a4ff");gradient.addColorStop(.58,flash?"#df9cff":"#a64cff");gradient.addColorStop(1,flash?"#a58dff":"#7047e8");context.fillStyle=gradient;context.fill();context.lineWidth=2.5;context.strokeStyle="#07141c";context.stroke();map.addImage(name,context.getImageData(0,0,size,size),{pixelRatio:2})}
    ...
    let mapReady=false;
    map.on("load",()=>{
      addTopStar();addTopStar("top-star-flash",true);
      ...
      map.addSource("active-cluster",{type:"geojson",data:{type:"FeatureCollection",features:[]}});
       ...
       map.addLayer({id:"cluster-count",type:"symbol",source:"clips",filter:["has","point_count"],layout:{"text-field":["to-string",["get","point_count"]],"text-font":["Arial"],"text-size":["step",["get","point_count"],13,100,12],"text-allow-overlap":true,"text-ignore-placement":true,"text-anchor":"center","text-justify":"center","text-letter-spacing":0,"text-rotation-alignment":"viewport","text-pitch-alignment":"viewport"},paint:{"text-color":"#fff","text-halo-color":"rgba(21, 10, 34, .55)","text-halo-width":.7}});
       map.addLayer({id:"clip-points",type:"circle",source:"clips",filter:["all",["!",["has","point_count"]],["==",["get","top"],false]],paint:{"circle-radius":["case",["get","linked"],5.5,4.5],"circle-color":["case",["get","linked"],"#bd5cff","#7c9299"],"circle-color-transition":{duration:120,delay:0},"circle-stroke-color":"#07141c","circle-stroke-width":1.5}});
