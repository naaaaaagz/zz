...
<head>
  ...
  <style>
    ...
    .live-button[hidden],.search-clear[hidden],.top-badge[hidden],.search-suggestions[hidden],.clip-list-dismiss[hidden]{display:none}
  </style>
</head>
<body>
  <main class="site-shell">
    ...
    <button class="clip-list-dismiss" id="clip-list-dismiss" type="button" aria-label="Lista bezárása" hidden></button>
  ...
  <script type="module">
    ...
    let topOnly=false,searchQuery="",searchFocused=false,suggestionCursor=0,searchOrigin=null,activeListPlace=null,hoveredListPlace=null,listTopOnly=false,listSort="date",listSortDirection="desc",currentVisible=[];
    ...
    const listShell=document.getElementById("clip-list-shell"),listPanel=document.getElementById("clip-list-panel"),listScroll=document.getElementById("clip-list-scroll"),listToggle=document.getElementById("clip-list-toggle"),listDismiss=document.getElementById("clip-list-dismiss"),listCount=document.getElementById("clip-list-count"),listTopToggle=document.getElementById("list-top-toggle"),listSortDate=document.getElementById("list-sort-date"),listSortName=document.getElementById("list-sort-name"),connector=document.getElementById("clip-connector");
    function setListOpen(open){listShell.classList.toggle("open",open);listDismiss.hidden=!open;listToggle.setAttribute("aria-expanded",String(open));listToggle.setAttribute("aria-label",open?"Lista bezárása":"Lista megnyitása");if(open)requestAnimationFrame(updateConnector);else{hoveredListPlace=null;connector.hidden=true}}
