...
<body>
  ...
  <script type="module">
    ...
    function makeTitleLabelBackground(){const width=40,height=30,canvas=document.createElement("canvas");canvas.width=width;canvas.height=height;const context=canvas.getContext("2d");if(!context)return null;context.beginPath();context.roundRect(1.5,1.5,width-3,height-3,8);context.fillStyle="rgba(11, 27, 36, 0.96)";context.fill();context.lineWidth=2;context.strokeStyle="rgba(192, 92, 255, 0.52)";context.stroke();return context.getImageData(0,0,width,height)}

    let mapReady=false;
    map.on("load",()=>{
      addTopStar();
      const titleBackground=makeTitleLabelBackground();if(titleBackground)map.addImage("title-label-background",titleBackground,{pixelRatio:2,stretchX:[[12,28]],stretchY:[[10,20]],content:[9,6,31,24]});
      ...
      map.addLayer({id:"clip-labels",type:"symbol",source:"clips",filter:["!",["has","point_count"]],layout:{visibility:"none","icon-image":"title-label-background","icon-text-fit":"both","icon-text-fit-padding":[5,8,5,8],"icon-anchor":"bottom","icon-offset":[0,-13],"icon-allow-overlap":true,"icon-ignore-placement":true,"text-field":["get","name"],"text-font":["Arial"],"text-size":11,"text-anchor":"bottom","text-offset":[0,-1.15],"text-max-width":18,"text-allow-overlap":true,"text-ignore-placement":true},paint:{"text-color":"#efffff","text-halo-color":"rgba(4, 11, 16, 0.45)","text-halo-width":.7,"text-halo-blur":.2}});
      ...
      map.addLayer({id:"active-cluster",type:"symbol",source:"active-cluster",layout:{"icon-image":["concat","cluster-",["to-string",["get","point_count"]]],"icon-size":1.08,"icon-allow-overlap":true}});
      const popup=new maplibregl.Popup({closeButton:false,closeOnClick:false,offset:12,className:"clip-map-tooltip"});
      function bindPointLayer(layerId){map.on("mouseenter",layerId,event=>{map.getCanvas().style.cursor="pointer";const feature=event.features&&event.features[0];if(!feature||feature.geometry.type!=="Point")return;mapHoveredPlace=places.find(item=>item.id===Number(feature.properties.id))||null;updateHighlightedPoint();if(!showTitles)popup.setLngLat(feature.geometry.coordinates).setText(feature.properties.name||"Névtelen klip").addTo(map)});map.on("mouseleave",layerId,()=>{map.getCanvas().style.cursor="";mapHoveredPlace=null;updateHighlightedPoint();popup.remove()});map.on("click",layerId,event=>{const id=Number(event.features&&event.features[0]&&event.features[0].properties.id),place=places.find(item=>item.id===id);if(place&&place.clipUrl)openClip(place)})}
      ...
      map.on("click",event=>{const interactiveFeatures=map.queryRenderedFeatures(event.point,{layers:["clip-clusters","clip-hit-area","clip-points","top-points","active-clip-point","active-top-point","active-cluster"]});if(!interactiveFeatures.length)setListOpen(false)});
