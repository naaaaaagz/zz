...
<body>
  ...
  <script type="module">
    ...
    function scheduleNodeGlow(){clearTimeout(glowTimer);cancelAnimationFrame(glowFrame);glowTimer=setTimeout(()=>{if(!map.getLayer("clip-points")||!map.hasImage("top-star"))return;const startedAt=performance.now(),normal=[189,92,255],highlight=[222,167,255],render=now=>{const progress=Math.min(1,(now-startedAt)/440),strength=Math.sin(progress*Math.PI),color=normal.map((channel,index)=>Math.round(channel+(highlight[index]-channel)*strength));map.setPaintProperty("clip-points","circle-color",["case",["get","linked"],"rgb("+color.join(",")+")","#7c9299"]);const glowingStar=makeTopStar(strength);if(glowingStar)map.updateImage("top-star",glowingStar);if(progress<1)glowFrame=requestAnimationFrame(render);else{map.setPaintProperty("clip-points","circle-color",["case",["get","linked"],"#bd5cff","#7c9299"]);const baseStar=makeTopStar();if(baseStar)map.updateImage("top-star",baseStar)}};glowFrame=requestAnimationFrame(render)},60)}
    ...
    map.on("load",()=>{
      ...
      map.addSource("active-cluster",{type:"geojson",data:{type:"FeatureCollection",features:[]}});
       ...
       map.addLayer({id:"clip-points",type:"circle",source:"clips",filter:["all",["!",["has","point_count"]],["==",["get","top"],false]],paint:{"circle-radius":["case",["get","linked"],5.5,4.5],"circle-color":["case",["get","linked"],"#bd5cff","#7c9299"],"circle-color-transition":{duration:0,delay:0},"circle-stroke-color":"#07141c","circle-stroke-width":1.5}});
