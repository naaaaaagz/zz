...
<body>
  ...
  <script type="module">
    ...
    function scheduleNodeGlow(){clearTimeout(glowTimer);cancelAnimationFrame(glowFrame);glowTimer=setTimeout(()=>{if(!map.getLayer("clip-points"))return;const startedAt=performance.now(),normal=[189,92,255],highlight=[222,167,255],render=now=>{const progress=Math.min(1,(now-startedAt)/440),strength=Math.sin(progress*Math.PI),color=normal.map((channel,index)=>Math.round(channel+(highlight[index]-channel)*strength));map.setPaintProperty("clip-points","circle-color",["case",["get","linked"],"rgb("+color.join(",")+")","#7c9299"]);if(progress<1)glowFrame=requestAnimationFrame(render);else map.setPaintProperty("clip-points","circle-color",["case",["get","linked"],"#bd5cff","#7c9299"])};glowFrame=requestAnimationFrame(render)},60)}
